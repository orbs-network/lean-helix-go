...
func TestReturnNilWhenNoViewChangeMessages(t *testing.T) {
	...
	VCMessage := builders.AViewChangeMessage(networkId, keyManager, memberId, 1, 2, nil)
...
func TestKeepOnlyMessagesWithBlock(t *testing.T) {
	...
	preparedMessages := &preparedmessages.PreparedMessages{
		...
		PrepareMessages: []*interfaces.PrepareMessage{
			builders.APrepareMessage(networkId, keyManager1, memberId1, 1, 2, block),
			builders.APrepareMessage(networkId, keyManager2, memberId2, 1, 2, block),
		},
	}

	VCMessage := builders.AViewChangeMessage(networkId, keyManager3, memberId3, 1, 2, preparedMessages)
...
func TestReturnBlockFromPPMWithHighestView(t *testing.T) {
	...
	preparedOnView3 := builders.CreatePreparedMessages(networkId, node3, []builders.Sender{node1, node2}, 1, 3, blockOnView3)

	VCMessageOnView3 := builders.AViewChangeMessage(networkId, node0.KeyManager, node0.MemberId, 1, 5, preparedOnView3)

	// view on view 8
	blockOnView8 := mocks.ABlock(interfaces.GenesisBlock)
	preparedOnView8 := builders.CreatePreparedMessages(networkId, node0, []builders.Sender{node1, node2}, 1, 8, blockOnView8)
	VCMessageOnView8 := builders.AViewChangeMessage(networkId, node2.KeyManager, node2.MemberId, 1, 5, preparedOnView8)

	// view on view 4
	blockOnView4 := mocks.ABlock(interfaces.GenesisBlock)
	preparedOnView4 := builders.CreatePreparedMessages(networkId, node0, []builders.Sender{node1, node2}, 1, 4, blockOnView4)
	VCMessageOnView4 := builders.AViewChangeMessage(networkId, node2.KeyManager, node2.MemberId, 1, 5, preparedOnView4)
